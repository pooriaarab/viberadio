...
describe('buildRecapScript', () => {
  ...
  it('skips unknown kinds that carry no human-readable detail', () => {
    ...
    expect(script).toBe(
      'Here is what happened: shipped it, and but this one has a note.',
    );
  });
  ...
  it('reads convenience top-level fields from a RawEvent', () => {
    ...
    expect(script).toBe(
      'Here is what happened: opened PR #99, and manual checkpoint note.',
    );
  });
  ...
  it('renders a podcast mode walkthrough', () => {
    ...
    expect(script).toBe(
      'Here is a walkthrough of what happened in this session: shipped it.',
    );
  });

  it('combines podcast style + podcast mode', () => {
    const script = buildRecapScript(
      [ev({ kind: 'tests-pass', payload: { count: 3 } })],
      { style: 'podcast', mode: 'podcast' },
    );
    expect(script).toBe(
